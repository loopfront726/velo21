...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/loopfront726/velo21/'..readfile('skidrewrite/profiles/commit.txt')..'/'..select(1, path:gsub('skidrewrite/', '')), true) end)
...
local vm = loadstring(downloadFile('skidrewrite/libraries/vm.lua'), 'vm')()
